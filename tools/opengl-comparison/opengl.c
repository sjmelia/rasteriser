...
static void draw_screen( void )
{
    ...
    gluProject(-1.0f, -1.0f, -4.0f, modelview, projection, viewport, &winX, &winY, &winZ);
    
    printf("RESULT OF GLUPROJECT\n viewport[0]:%d\n viewport[1]:%d\n viewport[2]:%d\n viewport[3]:%d\n winX: %G\n winY: %G\n winZ: %G\n\n", 
...
static void setup_opengl( int width, int height )
{
    ...
    GLint viewport[4];
    glGetIntegerv(GL_VIEWPORT, viewport);
    printf("view[0]: %f view[1]: %f view[2]: %f view[3]: %f\n",
            viewport[0], viewport[1], viewport[2], viewport[3]);
    /*
    ...
    GLdouble mtx[16];
    glGetDoublev(GL_PROJECTION_MATRIX, mtx);
    int i;
    for (i = 0; i < 16; i++)
    {
        if (i % 4 == 0)
